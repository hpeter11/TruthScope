...
# Map labels (assumes these are the same as used during training)
label_map = {"agree": 0, "disagree": 1, "discuss": 2, "unrelated": 3}
inv_label_map = {v: k for k, v in label_map.items()}

# Define the model name (should match the one you used for training)
...
# Load saved weights from your file (adjust the filename if needed)
...
def predict_stance(query: str, article: str) -> str:
    ...
    predict the stance. Returns one of 'agree', 'disagree', 'discuss', 'unrelated'.
    """
    # Concatenate query and article using the separator used during training.
